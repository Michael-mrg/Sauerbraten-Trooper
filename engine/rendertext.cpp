...
static void text_color2(char c, char *stack, int size, int &sp, bvec color, int a)
{
    stack[sp] = c;
    switch(c)
    {
...
void draw_text(const char *str, int left, int top, int r, int g, int b, int a, int cursor, int maxwidth) 
{
    ...
    #define TEXTCOLOR2(idx) text_color2(str[idx], colorstack, sizeof(colorstack), colorpos, color, a);
